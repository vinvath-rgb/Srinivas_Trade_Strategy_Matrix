# regime_matrix_app/orchestrator.py
...
def run_pipeline(
    ...
    *,
    ...
    regime_mode: str = "percentile",           # "percentile" or "threshold"
    low_pct: float = 0.40,
    high_pct: float = 0.60,
    k_mean: float = 1.2,
    k_median: float = 1.0,
    ...
    """
    End-to-end pipeline:
      1) Load prices (CSV or fetch)
      2) Build equal-weight portfolio curve
      3) Detect regimes
      4) Run strategies
      5) Build Strategy–Regime matrix
    Returns a dict with coverage_df, prices, eqw_curve, regimes, per_strategy, matrix_table
    """

    # -------------------------
    # 1) PRICES (CSV or fetch)
    # -------------------------
    if df_csv is not None and not df_csv.empty:
        # Normalize headers if LONG format provided
        ...
        if {"date", "ticker", "close"} <= set(cols_lower):
            ...
            # Assume WIDE: Date + one column per ticker
            df_csv = df_csv.copy()
            df_csv["Date"] = pd.to_datetime(df_csv["Date"])
        ...
        fetched = fetch_prices_for_tickers(tickers, start=start, end=end, logger=logger)
    ...
    # ---------------------------------------
    # 2) EQUAL-WEIGHT PORTFOLIO (allow gaps)
    # ---------------------------------------
    cl = prices.copy()
    if allow_partial:
        valid = cl.notna()
        ...
        port_ret = (weights * cl.pct_change()).sum(axis=1).fillna(0.0)
    else:
    ...
    if regime_mode == "percentile":
        regimes = compute_regimes(
            eqw_curve, mode="percentile", low_pct=low_pct, high_pct=high_pct
        )
    else:
        regimes = compute_regimes(
            eqw_curve, mode="threshold", k_mean=k_mean, k_median=k_median
        )
    ...
    # --------------------------------------
    # 5) STRATEGY–REGIME MATRIX (CAGR by bin)
    # --------------------------------------
    regime_bucket = (
        regimes["Regime_Median"] if "Regime_Median" in regimes else regimes["Regime_Mean"]
    )
    ...
    for key, res in per_strategy.items():
        ...
        def cagr(x: pd.Series) -> float:
            n = len(x)
            return (1.0 + x).prod() ** (252.0 / n) - 1.0 if n > 0 else float("nan")
